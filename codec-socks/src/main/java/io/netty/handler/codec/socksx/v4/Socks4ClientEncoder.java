 ...
 */
@Sharable
public class Socks4ClientEncoder extends MessageToByteEncoder<Socks4CommandRequest> {
    ...
    protected void encode(ChannelHandlerContext ctx, Socks4CommandRequest msg, ByteBuf out) throws Exception {
        ...
        if (NetUtil.isValidIpV4Address(msg.dstAddr())) {
            ...
            out.writeBytes(msg.userId().getBytes(CharsetUtil.US_ASCII));
            out.writeByte(0);
        } else {
            out.writeBytes(IPv4_DOMAIN_MARKER);
            out.writeBytes(msg.userId().getBytes(CharsetUtil.US_ASCII));
            out.writeByte(0);
            out.writeBytes(msg.dstAddr().getBytes(CharsetUtil.US_ASCII));
            out.writeByte(0);
        }
    }
}
